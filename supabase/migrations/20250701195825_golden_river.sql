/*
  # Complete Airport Operations Platform

  1. New Tables
    - `departments` - Airport departments (Security, Operations, Maintenance, Customer Service)
    - `shifts` - Shift management and handovers
    - `incidents` - Incident reporting and tracking
    - `tasks` - Task assignment and management
    - `equipment` - Equipment monitoring and maintenance
    - `announcements` - System-wide announcements
    - `contacts` - Staff directory with emergency contacts

  2. Enhanced Tables
    - `user_profiles` - Added department_id, role, phone columns
...
DROP POLICY IF EXISTS "Users can view channels in their enterprise" ON channels;
DROP POLICY IF EXISTS "Users can create channels in their enterprise" ON channels;
DROP POLICY IF EXISTS "Channel creators and admins can update channels" ON channels;
DROP POLICY IF EXISTS "Super admins have full access to channels" ON channels;
DROP POLICY IF EXISTS "Users can view messages in channels they're members of" ON messages;
DROP POLICY IF EXISTS "Users can send messages to channels they're members of" ON messages;
DROP POLICY IF EXISTS "Users can update their own messages" ON messages;
DROP POLICY IF EXISTS "Users can delete their own messages" ON messages;
DROP POLICY IF EXISTS "Super admins have full access to messages" ON messages;
DROP POLICY IF EXISTS "Users can view members of channels they're in" ON channel_members;
DROP POLICY IF EXISTS "Channel admins can manage members" ON channel_members;
DROP POLICY IF EXISTS "Users can join public channels in their enterprise" ON channel_members;
DROP POLICY IF EXISTS "Super admins have full access to channel members" ON channel_members;
...
CREATE TABLE IF NOT EXISTS equipment (
  ...
  department_id uuid REFERENCES departments(id),
  name text NOT NULL,
  type text NOT NULL, -- e.g., 'vehicle', 'scanner', 'radio'
  model text,
  serial_number text,
  location text,
  status text NOT NULL DEFAULT 'operational' CHECK (status IN ('operational', 'maintenance', 'out_of_service', 'repair')),
...
CREATE POLICY "enterprise_users_view_equipment"
  ON equipment FOR SELECT TO authenticated
  USING (enterprise_id IN (SELECT enterprise_id FROM user_profiles WHERE id = auth.uid()));

CREATE POLICY "users_update_equipment"
  ON equipment FOR UPDATE TO authenticated
  USING (enterprise_id IN (SELECT enterprise_id FROM user_profiles WHERE id = auth.uid()));

CREATE POLICY "super_admins_manage_equipment"
...
-- RLS Policies for channels (recreated without conflicts)
...
-- RLS Policies for messages (recreated without conflicts)
...
-- RLS Policies for channel_members (recreated without conflicts)
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_equipment_updated_at') THEN
    CREATE TRIGGER update_equipment_updated_at
      BEFORE UPDATE ON equipment FOR EACH ROW
      EXECUTE FUNCTION update_updated_at_column();
  END IF;
END $$;
...
BEGIN
  ...
  FOR enterprise_record IN SELECT * FROM enterprises WHERE status = 'active' LOOP
    ...
    INSERT INTO equipment (enterprise_id, department_id, name, type, status, location)
    VALUES 
      (enterprise_record.id, dept_security, 'X-Ray Scanner #1', 'scanner', 'operational', 'Terminal A Security'),
      (enterprise_record.id, dept_security, 'Metal Detector #3', 'scanner', 'operational', 'Terminal B Security'),
      (enterprise_record.id, dept_operations, 'Baggage Tug #12', 'vehicle', 'operational', 'Ramp Area'),
      (enterprise_record.id, dept_operations, 'Ground Power Unit #5', 'equipment', 'maintenance', 'Gate 15'),
      (enterprise_record.id, dept_maintenance, 'Radio System', 'communication', 'operational', 'Control Tower')
